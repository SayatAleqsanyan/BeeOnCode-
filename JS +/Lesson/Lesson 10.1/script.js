...
// ----- Mouse Events & First Step Check-----------------------------------------------------------------------------------------------------------------------------
game.addEventListener("click", (event) => {
  event.preventDefault();
  if (isLoesed) return;
  if (finsishing) return;
  openCellLists = document.querySelectorAll(".open");
  if (openCellLists.length === 0 && event.target.classList.contains("bomb")) {
    event.target.removeAttribute("mines");
...
function finishGame() {
  ...
  if (closedCells === bombs) {
    ...
    const openedSafeCells =
      document.querySelectorAll(".open:not(.bomb)").length;
